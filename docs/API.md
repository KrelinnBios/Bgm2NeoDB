...
实际调用：`GET /v0/me` 自动识别当前用户，`GET /v0/users/{username}/collections?limit=50&offset=...` 分页读取。分页上限为 50。每页所有原始字段均保留，重复条目或不完整分页会阻止替换当前快照。
...
每个程序单进程、单任务；网络请求受限并发执行（解析最多 4、写入最多 8），且按全局请求间隔限速。原始快照用临时文件原子替换，数据库事务保证完整扫描切换的原子性。POST 发出前持久记录 `writing`，成功后回读确认才记 `migrated`。

服务端没有暴露可用的条件写入版本号，因此“回读当前值 → POST”之间仍存在很短的竞争窗口。迁移过程中请避免同时编辑同一条目的 NeoDB 收藏。程序不声称提供服务端原子比较并交换。

测试覆盖映射、隐私、合并跳转、限流、凭证失效、断点恢复、已提交但断连的请求、回读不一致、跨账号隔离、本地页面防跨站请求和与快照的字段契约。无真实账号数据被用于这些测试。
